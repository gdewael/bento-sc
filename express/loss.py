...
class PoissonNLL(ExpressLoss):
    def __init__(self, dim, lib_norm = False, reduction="mean", omit_last_term=True, zero_truncated = False,):
        super().__init__(dim, 1, reduction=reduction)
    ...
    def loss(self, inputs, targets):
        if self.zero_trunc:
            loss = (inputs.exp() - 1 + 1e-8).log() - targets * (inputs + 1e-8).log()
        else:
...
class NegativeBinomialNLL(ExpressLoss):
    def __init__(self, dim, lib_norm = False, n_genes = 19331, fixed_dispersion = False, reduction="mean", omit_last_term=True, zero_truncated = False,):
    ...
    def loss(self, mus, log_thetas, targets):
        ...
        loss = torch.lgamma(thetas) - torch.lgamma(targets + thetas)+ (thetas + targets) * (thetas + mus).log() - thetas * log_thetas - targets * mus.log()
        if not self.zero_trunc:
            loss += (1 - (thetas/(thetas+mus)) ** thetas).log()
    ...
    def forward(self, x, targets, gene_ids = None):
        mus, log_thetas = self.predict(self, x, gene_ids = gene_ids, libsize = targets.sum())
        return self.loss(mus, log_thetas, targets)



class ZeroInflatedNegativeBinomialNLL(NegativeBinomialNLL):
    def __init__(self, dim, lib_norm = False, n_genes = 19331, fixed_dispersion = False, reduction="mean", omit_last_term=True):
        super().__init__(dim, lib_norm = lib_norm, n_genes = n_genes, fixed_dispersion = fixed_dispersion, reduction=reduction, omit_last_term=omit_last_term)
    ...
    def loss(self, mus, log_thetas, pis, targets):
        ...
        NLLifzero = F.softplus(-pis) - F.softplus(-pis + thetas * (log_thetas - (thetas + mus).log()))
